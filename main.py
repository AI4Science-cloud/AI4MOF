...
from MOF_graph import models, process, training
# from matdeeplearn.process import process_
import warnings
...
def main():
    ...
    ##Predicting from a trained model
        ...
        train_error,R2_Test, RMSE_Test = training.predict(
            dataset, config["Training"]["loss"], config["Job"]
        )
        print("Test Error: {:.5f}".format(train_error))
        print("R2_Test: {:.5f}".format(R2_Test))
        print("RMSE_Test: {:.5f}".format(RMSE_Test))
