...
class GCN(torch.nn.Module):
    ...
    def forward(self, data):
        if hasattr(data,'edge_index_1'):
            ...
            #edge_index = data.edge_index_1
            #edge_attr = torch.cat([data.edge_attr_1, data.edge_attr_2, data.edge_attr_3], 0)
        #data.edge_weight = torch.cat([data.edge_attr_1, data.edge_attr_2, data.edge_attr_3], 0)
        ...
        for i in range(0, len(self.conv_list)):
            if len(self.pre_lin_list) == 0 and i == 0:
                if self.batch_norm == "True":
                    out = self.conv_list[i](data.x, edge_index, data.edge_weight)
                    out = self.bn_list[i](out)
                else:
                    out = self.conv_list[i](data.x, edge_index, data.edge_weight)
            else:
                if self.batch_norm == "True":
                    out = self.conv_list[i](out, edge_index, data.edge_weight)
                    out = self.bn_list[i](out)
                else:
                    out = self.conv_list[i](out, edge_index, data.edge_weight)            
            out = getattr(F, self.act)(out)
