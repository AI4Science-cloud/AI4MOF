    ...
    def __init__(
        ...
        **kwargs
        ...
        for i in range(gc_count):
            conv = InteractionBlock(gc_dim, 3, dim3, cutoff)
    ...
    def forward(self, data):
        ...
        for i in range(0, len(self.conv_list)):
            if len(self.pre_lin_list) == 0 and i == 0:
                if self.batch_norm == "True":
                    out = data.x + self.conv_list[i](data.x, edge_index, data.edge_weight, edge_attr)
                    out = self.bn_list[i](out)
                else:
                    out = data.x + self.conv_list[i](data.x, edge_index, data.edge_weight, edge_attr)
            else:
                if self.batch_norm == "True":
                    out = out + self.conv_list[i](out, edge_index, data.edge_weight, edge_attr)
                    out = self.bn_list[i](out)
                else:
                    out = out + self.conv_list[i](out, edge_index, data.edge_weight, edge_attr)            
            #out = getattr(F, self.act)(out)
